...
elseif r==2 & c==2
  ...
  if length(prop.color)>0
    h1=plot(scale*(x0+k*x),scale*(y0+k*y),'color',prop.color,'linewidth',prop.linewidth);
  else
    h1=plot(scale*(x0+k*x),scale*(y0+k*y),'linewidth',prop.linewidth);
  end
